...
const getLateralContingencyManoeuvres = (
  ...
  if (["ROTORCRAFT", "MULTIROTOR"].includes(droneType.value)) {
    result.push({ 
      value: "STOPPING", 
      label: translate("contingencyManoeuvres.stopping", "Stopping")
    });
  }
  ...
  if (droneType.value === "FIXEDWING") {
    result.push({ 
      value: "TURN_180", 
      label: translate("contingencyManoeuvres.turn180", "180° turn")
    });
  }

  result.push({ 
    value: "PARACHUTE_TERMINATION", 
    label: translate("contingencyManoeuvres.triggerParachute", "Trigger parachute")
  });
...
const getVerticalContingencyManoeuvres = (
  ...
  if (["ROTORCRAFT", "MULTIROTOR"].includes(droneType.value)) {
    result.push({ 
      value: "ENERGY_CONVERSION", 
      label: translate("contingencyManoeuvres.energyConversion", "Energy conversion")
    });
  }
  ...
  if (droneType.value === "FIXEDWING") {
    result.push({ 
      value: "CIRCULAR_PATH", 
      label: translate("contingencyManoeuvres.circularPath", "Circular path")
    });
  }
...
const getMethodOfTermination = (
  ...
  if (!hasParachute) {
    if (droneType.value === "FIXEDWING") {
      options.push(
        { 
          value: "OFF_NO_GLIDING", 
          label: translate("terminationMethods.powerOffNoGliding", "Power off (no gliding)")
        },
        { 
          value: "OFF_GLIDING", 
          label: translate("terminationMethods.powerOffGliding", "Power off (gliding)")
        },
      );
    } else {
      options.push(
        { 
          ...
          label: translate("terminationMethods.simplifiedApproach", "Simplified approach")
        },
        { 
          value: "BALLISTIC_APPROACH", 
          label: translate("terminationMethods.ballisticApproach", "Ballistic approach")
        },
      );
    }
  }
  ...
  options.push({ 
    value: "PARACHUTE", 
    label: translate("terminationMethods.parachuteTermination", "Parachute termination")
  });
