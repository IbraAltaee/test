...
const InputForm: React.FC<InputFormProps> = ({
  ...
  const handleDroneTypeChange = useCallback(
    ...
    [clearFieldError, data.maxOperationalSpeed, handleDataChange],
  );
    ...
    (selectedOption: SingleValue<OptionType>) => {
      ...
      if (
        ...
        handleDataChange(String(data.windSpeed), "windSpeed", {
          min: 3,
          custom: (v) =>
            v < 3 ? "Wind speed below 3 m/s is not considered realistic" : null,
        });
      }
      if (
    ...
    try {
      ...
      if (error.requiresRecaptcha) {
        if (error.recaptchaExpired) {
          resetRecaptcha();
          toast.error(
            "reCAPTCHA verification expired. Please verify again and resubmit.",
          );
        } else {
          setShowRecaptcha(true);
          toast.error(
            error.error || "Please complete the reCAPTCHA verification",
          );
        }
      } else {
  ...
  const handleSubmit = useCallback(
    ...
    [isInputValid, hasFormChanged, hasLastSubmission, submitFormData],
  );
      ...
      {username && (
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <input
            ...
            onChange={(e) => droneOps.setConfigName(e.target.value)}
