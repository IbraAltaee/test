...
const AdminDashboard: React.FC = () => {
  ...
  const validateZone = (zone: Zone): { isValid: boolean; errors: string[] } => {
    ...
    if (validPoints.length < 3) {
      errors.push(
        `At least 3 valid points are required (currently: ${validPoints.length})`,
      );
    }
    ...
    if (incompletePoints.length > 0) {
      errors.push(
        `${incompletePoints.length} incomplete point${
          incompletePoints.length > 1 ? "s" : ""
        } found`,
      );
    }
    ...
    if (defaultPoints.length > 0) {
      toast.error(
        `${defaultPoints.length} point${
          defaultPoints.length > 1 ? "s" : ""
        } cannot be at coordinates (0,0)`,
      );
    }
