export const generateEmailBody = (data: any) => {
    const stripIndent = (txt: string) =>
        txt.replace(/^[ \t]+/gm, "").replace(/\n{3,}/g, "\n\n").trim();

    const lateralCVSection = (() => {
        const { contingencyManoeuvre, rollAngle, pitchAngle, timeToOpenParachute, lateralExtension } = data.lateralCV;
        ...
        lines.push(`- Minimum lateral dimension: ${parseFloat(lateralExtension).toFixed(2)} m`);

        return lines.join("\n");
    })();

    const verticalCVSection = (() => {
        const { contingencyManoeuvre, responseHeight, timeToOpenParachute, heightContingencyManoeuvre, minVerticalDimension} = data.verticalCV;
        const hasLateralParachute = data.lateralCV.contingencyManoeuvre === "PARACHUTE_TERMINATION";
        ...
        if (contingencyManoeuvre === "PARACHUTE_TERMINATION" && !hasLateralParachute) {
            lines.push(`- Time to open parachute: ${timeToOpenParachute} s`);
        }

        lines.push(`- Minimum vertical dimension: ${parseFloat(minVerticalDimension).toFixed(2)} m`);

        return lines.join("\n");
    })();

    const grbSection = (() => {
        const {termination, timeToOpenParachute, maxPermissibleWindSpeed, rateOfDescent, glideRatio, minLateralDimension} = data.grb;
        const hasLateralParachute = data.lateralCV.contingencyManoeuvre === "PARACHUTE_TERMINATION";
        const hasVerticalParachute = data.verticalCV.contingencyManoeuvre === "PARACHUTE_TERMINATION";
        ...
        if (termination === "PARACHUTE" && !hasLateralParachute && !hasVerticalParachute) {
            lines.push(`- Time to open parachute: ${timeToOpenParachute} s`);
        ...
        lines.push(`- Minimum lateral dimension: ${parseFloat(minLateralDimension).toFixed(2)} m`);
