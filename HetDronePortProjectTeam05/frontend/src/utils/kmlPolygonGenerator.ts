...
export const toCoordinatesString = (
  ...
  if (topAltitude > baseAltitude) {
    const bottomCoords = closedPoints.map(p => `${p.lng},${p.lat},${baseAltitude}`).join(" ");
    const topCoords = [...closedPoints].map(p => `${p.lng},${p.lat},${topAltitude}`).join(" ");
...
export const generatePolygonData = (
  ...
  const adjacentVolumePolygon = turf.buffer(
    originalPolygon,
    adjacentVolume,
    { units: "meters" },
  );
...
export const generateKmlFromFlightGeography = (flightGeography: {
  lat: number;
  lng: number;
}[]) => {
